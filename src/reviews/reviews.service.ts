...
export class ReviewsService {
  ...
  async findReviewedProductIds(userId: string) {
    const reviews = await this.prisma.review.findMany({
      where: {
        userId,
        source: 'customer',
  ...
  async getOrderItemReviewStatus(
    user: AuthenticatedUser,
    items: { orderNumber: string; productId: string }[],
  ) {
    ...
    const reviewedKeys = new Set(
      reviews
        ...
        .map(
          (review) =>
            `${review.orderId}:${this.normalizeProductId(review.productId)}`,
        ),
    );

    const legacyReviewedProducts = new Set(
      reviews
        .filter((review) => !review.orderId)
        .map((review) => this.normalizeProductId(review.productId)),
    );

    return normalizedItems.reduce<Record<string, { reviewed: boolean }>>(
      (acc, item) => {
  ...
  async getEligibility(
    ...
    if (orderNumber?.trim()) {
      const order = orders.find(
        (entry) => entry.orderNumber === orderNumber.trim(),
      );
      ...
      return this.evaluateOrderItemEligibility(
        order,
        normalizedProductId,
        user.id,
      );
    ...
    for (const order of orders) {
      ...
      const existing = await this.hasReviewForOrderItem(
        user.id,
        order.id,
        normalizedProductId,
      );
      ...
      if (!existing) {
        return this.buildEligibility({
          ...
          message:
            'Aap ka parcel deliver ho chuka hai. Ab review submit kar sakte hain.',
        });
      }
    }
    ...
    const hadDeliveredOrder = orders.some(
      (order) =>
  ...
  async createForCustomer(user: AuthenticatedUser, dto: CreateReviewDto) {
    ...
    if (!eligibility.canReview) {
      throw new BadRequestException(
        eligibility.message || 'Review submit nahi ho sakti.',
      );
    }
    ...
    if (!customerName) {
      throw new BadRequestException('Pehle apna profile name set karein.');
    }
  ...
  async createForAdmin(dto: CreateAdminReviewDto) {
    const product = await this.prisma.product.findUnique({
  ...
  async updateAdmin(id: string, dto: UpdateReviewDto) {
  ...
  private buildEligibility(payload: ReviewEligibility): ReviewEligibility {
    return payload;
  }

  private orderItemKey(orderNumber: string, productId: string) {
  ...
  private async hasReviewForOrderItem(
    userId: string,
    orderId: string,
    productId: string,
  ) {
  ...
  private async evaluateOrderItemEligibility(
    order: Order,
    productId: string,
    userId: string,
  ...
  private orderHasProduct(order: Order, productId: string) {
    const target = this.normalizeProductId(productId);
    return this.parseOrderItems(order).some(
      (item) => this.normalizeProductId(item.productId) === target,
    );
  }
  ...
  private async findReviewRecord(id: string): Promise<Review> {
